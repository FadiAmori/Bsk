...
const FournisseurComponent = () => {
  ...
  const handleView = async (fournisseur) => {
    try {
      const res = await axios.get(`http://localhost:5000/api/fournisseurs/${fournisseur._id}`);
      setCurrentFournisseur({
        ...res.data,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response;
      if (isEditing) {
        response = await axios.put(`http://localhost:5000/api/fournisseurs/${_id}`, payload);
      } else {
        response = await axios.post("http://localhost:5000/api/fournisseurs", payload);
      }
      fetchFournisseurs();
      setShowModal(false);
    } catch (err) {
