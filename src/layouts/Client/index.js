...
const ClientComponent = () => {
  ...
  const handleView = async (client) => {
    try {
      const res = await axios.get(`http://localhost:5000/api/clients/${client._id}`);
      setCurrentClient({
        ...res.data,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response;
      if (isEditing) {
        response = await axios.put(`http://localhost:5000/api/clients/${_id}`, payload);
      } else {
        response = await axios.post("http://localhost:5000/api/clients", payload);
