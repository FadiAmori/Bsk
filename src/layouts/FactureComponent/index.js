...
const FactureComponent = () => {
  ...
  const handleSubmitFacture = async (e) => {
    ...
    try {
      let response;
      if (isEditing) {
        response = await axios.put(`http://localhost:5000/api/factures/${_id}`, payload);
      } else {
        response = await axios.post("http://localhost:5000/api/factures", payload);
      }
      const populatedFacture = await axios.get(
        `http://localhost:5000/api/factures/${response.data._id}`
      );
  ...
  const generatePDF = (facture) => {
    try {
      ...
      const addHeader = () => {
        ...
        doc.setFontSize(16);
        doc.text("Facture Client", pageWidth / 2, logoY + logoHeight + 10, { align: "center" });
        doc.setFontSize(12);
