...
const BonDeSortieComponent = () => {
  ...
  const handleEditBon = async (bon) => {
    setIsEditing(true);
    try {
      const res = await axios.get(`http://localhost:5000/api/bons-de-sortie/${bon._id}`);
  ...
  const handleDeleteBon = async (id) => {
    if (window.confirm("Êtes-vous sûr de vouloir supprimer ce bon de sortie ?")) {
      try {
        await axios.delete(`http://localhost:5000/api/bons-de-sortie/${id}`);
  ...
  const handleSubmitBon = async (e) => {
    ...
    try {
      let response;
      if (isEditing) {
        response = await axios.put(`http://localhost:5000/api/bons-de-sortie/${_id}`, payload);
      } else {
        response = await axios.post("http://localhost:5000/api/bons-de-sortie", payload);
      }
      console.log("Create/Update response:", response.data);
      const populatedBon = await axios.get(
        `http://localhost:5000/api/bons-de-sortie/${response.data._id}`
      );
