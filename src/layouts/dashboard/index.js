...
function Dashboard() {
    ...
    const fetchData = async () => {
      ...
      try {
        const [
          ...
          axios.get("https://bskbackend.onrender.com/api/clients"),
          axios.get("https://bskbackend.onrender.com/api/fournisseurs"),
          axios.get("https://bskbackend.onrender.com/api/produits"),
          axios.get("https://bskbackend.onrender.com/api/factures"),
          axios.get("https://bskbackend.onrender.com/api/factureAchats"),
          axios.get("https://bskbackend.onrender.com/api/resumes-comptables"),
          axios.post("https://bskbackend.onrender.com/api/resumes-comptables"), // Generate résumés
        ]);
  ...
  const filteredResumes = resumesComptables.filter((resume) => {
  ...
  const handleSaveFraisGeneraux = async (id) => {
    try {
      ...
      await axios.put(`https://bskbackend.onrender.com/api/resumes-comptables/${id}`, {
        fraisGeneraux,
      });
  ...
  const clientPurchaseData = clients
    .map((client) => {
      const totalMontantTTC = factures
  ...
  const fournisseurPurchaseData = fournisseurs
    .map((fournisseur) => {
  ...
  const productSalesData = produits
    .map((produit) => {
      const totalQuantity = factures.reduce((sum, f) => {
        const productItems =
          f.liste?.filter((item) => (item.produit?._id || item.produit) === produit._id) || [];
        return sum + productItems.reduce((acc, item) => acc + (item.quantite || 0), 0);
      }, 0);
      return { name: produit.nomProduit || "Produit Inconnu", value: totalQuantity };
    })
    .filter((d) => d.value > 0);
  ...
  const getInvoicesPerDay = () => {
    ...
    const clientFactures = factures.filter((f) => (f.client?._id || f.client) === selectedClient);
    const invoiceCounts = {};
